...
public class WebSecurityConfig {
	...
	public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http, AuthenticationManager authManager) {
		return http.authorizeExchange().pathMatchers(HttpMethod.OPTIONS).permitAll()
				...
				.pathMatchers("/orders/order-completed/{deliveryGuyEmail}/{email}/{orderId}").hasAnyAuthority("ROLE_DELIVERY")
				.pathMatchers("/orders/order-accepted/{deliveryGuyEmail}/{email}/{orderId}").hasAnyAuthority("ROLE_DELIVERY")
